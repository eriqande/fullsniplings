...
plot(colony_tab$CumulCorrectSibs, colony_tab$CumulSibsInIncorrectSibships, 
     xlim = c(0, max(crtab$CumulCorrectSibs)))
lines(crtab$CumulCorrectSibs, crtab$CumulSibsInIncorrectSibships)
```
...
```{r}
plot(crtab$CumulCorrectSibs, cumsum(crtab$NumSibs * (crtab$AveNumNonSibs > 0.01)),
     type = "l", lwd=2, col = "orange")
lines(colony_tab$CumulCorrectSibs, cumsum(colony_tab$NumSibs * (colony_tab$AveNumNonSibs > 0.01)))
```
