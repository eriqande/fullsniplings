...
fsp <- full_sibling_pair_greqs(gf, mu = .005)
fsp[,]
```
...
One of my ideas for the program is to only propose moving individuals into sibships that already include an individual
that they have chance of being a full sib with.  In order to figure out who we will say they "have a chance of being a
full sib with" we will simulate a bunch of full sib genotypes from the underlying allele frequencies in the population
and look at the distribution of their log likelihood ratios under the hypotheses of full sibling versus unrelated.

So, we need to build those up.  Here we use a function that returns full sibling genotype probabilities, taking account
of genotyping error:
